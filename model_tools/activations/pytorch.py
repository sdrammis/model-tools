...
def load_preprocess_images(image_filepaths, image_size, as_rgb=True, **kwargs):
    images = load_images(image_filepaths, as_rgb)
    images = preprocess_images(images, image_size=image_size, as_rgb=as_rgb, **kwargs)
    return images


def load_images(image_filepaths, as_rgb):
    return [load_image(image_filepath, as_rgb) for image_filepath in image_filepaths]


def load_image(image_filepath, as_rgb):
    with Image.open(image_filepath) as pil_image:
        if 'L' not in pil_image.mode.upper() and 'A' not in pil_image.mode.upper() \
            ...
            if as_rgb:
                ...
                return ImageOps.grayscale(pil_image.copy())
...
def preprocess_images(images, image_size, as_rgb, **kwargs):
    preprocess = torchvision_preprocess_input(image_size, as_rgb, **kwargs)
    images = [preprocess(image) for image in images]
    images = np.concatenate(images)
    return images


def torchvision_preprocess_input(image_size, as_rgb, **kwargs):
    ...
    return transforms.Compose([
        ...
        torchvision_preprocess(as_rgb, **kwargs),
    ])


def torchvision_preprocess(as_rgb, normalize_mean=(0.485, 0.456, 0.406), normalize_std=(0.229, 0.224, 0.225)):
    from torchvision import transforms
    if as_rgb:
        return transforms.Compose([
            transforms.ToTensor(),
